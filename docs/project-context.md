...
- **App-wide Undo UI**: activity/history `UndoToast` rollout, supersession UX, and the bulk-delete cap (local-first tasks 3.3–3.6). Manage Categories has its category-scoped countdown, durable restore, and system Undo registration (archived `add-category-management`); entry deletions from ActivityDetail restore via shake-to-undo with no toast (archived `edit-entry-from-activity-detail`) — the toast itself stays deferred.
...
- **Activity deletion UI** — `LocalStore.deleteActivity` (cascade to entries, first-class outbox delete) has no caller in the UI; a separate `delete-activity` change is planned (destructive confirm: "Delete Running and its N entries (Xh tracked)?", not undoable).
...
- **Active deltas** (`openspec/changes/<change>/specs/<capability>/spec.md`): proposed ADDED/MODIFIED requirements not yet in the baseline. No active change right now — check `openspec list`.
