...
- **Active deltas** (`openspec/changes/<change>/specs/<capability>/spec.md`): proposed ADDED/MODIFIED requirements not yet in the baseline. Active change right now: `unify-catalog-deletion` (one deletion grammar for entries/activities/categories) — check `openspec list`.
