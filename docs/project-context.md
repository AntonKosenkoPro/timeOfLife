...
- **Track experience** — a three-tab shell (Track / History / Insights) with a centered numeric timer, platform-native Activity search for preparation (browse / filter / quick-create), an adaptive dual-flow layout (48 pt shared spacer cap, central separator, scroll fallback) with a state-invariant main-action frame (reserved idle preparation slot, hidden-but-reserved Recents, fixed-height action slot, top-first error yield), Recents chips (cap 6, most-recently-used first, first-Category icons, filled accent selected presentation) and no editing affordance on Track, a Profile destination, and a compact cross-tab running timer. Categories are optional Activity metadata, seeded locally and managed from Profile. Landing is the result of the archived OpenSpec changes `redesign-track-experience`, `keep-timer-position-on-stop`, `unify-activity-preparation-flow`, `refine-selected-activity-from-track`, `add-category-management`, and `refine-track-recents`.
- **History** — a read-only, day-grouped list of committed time entries in the History tab (archived `history-entry-list`, nav bar made persistent by `revert-history-nav-collapse`): `HistoryView` + `HistoryViewModel` with `EntryRow` rows (Variant H layout: first-category icon, name + natural-language duration, category names + "via <Source>" provenance + timeframe), relative-then-absolute day labels, per-day totals shown on the elevated (pinned) day header, a permanently visible nav bar, and the compact cross-tab timer preserved. Tapping an entry row opens the **Activity detail sheet** (`Features/ActivityDetail`, the `activity-detail-sheet` change): medium→large detents, identity header with the all-time tracked total, "Edit Activity" stacking the existing editor, and the activity's complete day-grouped committed entry list on tappable rows. Each row opens the unified entry form as a full-screen cover (archived `edit-entry-from-activity-detail`): EDIT for `manual` entries (prefilled activity/starts/ends, Save via LWW `updateEntry`), LOCKED read-only for imported entries (delete only), bottom destructive Delete entering the durable undo buffer with shake-to-undo and no toast. In-progress entries never appear in the detail list and never contribute to totals. Manual logging (`Features/ManualEntry`, archived `add-manual-entry`): a Calendar-style Log Time sheet (Activity + Starts + Ends, inline pickers, Add validity gate) from the History toolbar `[+]` and the detail sheet's `Log time` action, saving committed `manual` entries through the outbox. Deferred follow-ups (filtering, History-row swipe delete, UndoToast) live in `docs/history-roadmap.md`.
- **Insights** — a read-only, mirror-only breakdown of committed time in the Insights tab (`Features/Insights`, `insights-v1-breakdown` change): a `Today | This week | All time` period switch (default `This week`), a hero period total, a `By category | By activity` lens toggle (default `By category`), and proportional rows with max-scaled bars. Category rows attribute the full duration to every currently-attached category (rows may sum above the hero) with a one-line footnote naming the rule; rows are not tappable; no percentages, targets, or comparisons. Aggregation is pure and in-memory over the existing `LocalStore` reads; no navigation, drill-in, or filtering yet.
...
- **GRDB `LocalStore`** (D1): SQLite via GRDB.swift in the App Group shared container `group.com.antonkosenko.timeoflifeapp` (entitlement in `ios/TimeOfLife/TimeOfLife/Configuration/TimeOfLife.entitlements`), data protection `.completeUntilFirstUserAuthentication` (default). State tables (activities, categories, entries, activity_categories, timer_state), the transactional outbox, the durable undo buffer, and per-resource sync cursors. Accessible cross-process (widgets, Screen Time extension, lock-screen Control intents) and after first-unlock-since-boot. `LocalStore` is the **single chokepoint for all mutations** — no raw GRDB writes outside it (lint/review rule).
...
- **Activity deletion UI** — delivered by `unify-catalog-deletion`: bottom Delete in `ActivityEditorView` (edit mode) with scope confirmation ("Entries: N, total: T"), `LocalStore.deleteActivityUndoable` (cascade snapshot of activity + entries, running-timer block), editor dismiss + presenter settle, system-prompt undo on Track/History. Deleting the running activity is blocked with a message.
...
ios/TimeOfLife/          SwiftUI app (iOS 15+), XcodeGen-managed (project.yml)
  ...
  TimeOfLife/Features/ActivityDetail/ Activity detail sheet presented from a History entry tap (activity-detail-sheet change)
