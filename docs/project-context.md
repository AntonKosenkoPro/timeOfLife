...
- **Archives** (`openspec/changes/archive/<change>/`): completed changes. Their delta specs were folded into the baselines by `openspec archive`; the folder is history. Today: `redesign-track-experience`, `keep-timer-position-on-stop`, `unify-activity-preparation-flow`, `refine-selected-activity-from-track`, `add-category-management`, `integrate-app-icon`, `collapsing-editor-sheet-headers`, `refine-track-recents`, `history-entry-list`, `revert-history-nav-collapse`, `local-first-sync-architecture`, `activity-detail-sheet`.
