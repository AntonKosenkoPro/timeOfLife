...
- **Baseline specs** (`openspec/specs/<capability>/spec.md`): the current merged contract, in force. Thirteen baselines exist today: `activity-detail-sheet`, `app-icon`, `app-shell`, `category-management`, `editor-sheet-ux`, `entry-editor`, `entry-provenance`, `history-entry-list`, `local-first-store`, `lock-screen-controls`, `manual-entry`, `sync-client`, and `timer-capture-experience`. They were synced from archived changes. **Rule: never edit a baseline spec directly** — behavior changes go through a change with a delta spec.
