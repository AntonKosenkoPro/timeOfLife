...
- **Track experience** — a three-tab shell (Track / History / Insights) with a centered numeric timer, platform-native Activity search for preparation (browse / filter / quick-create), a **Refine** action beside the selected Activity for in-place editing, a Profile destination, and a compact cross-tab running timer. Landing is the result of the archived OpenSpec changes `redesign-track-experience`, `keep-timer-position-on-stop`, `unify-activity-preparation-flow`, and `refine-selected-activity-from-track`.
...
The device is the **source of truth**; the backend is an **optional relay**; sync is a transport feature that activates on sign-in — not a prerequisite for using the app. The app launches into the Track shell without sign-in; auth is an optional "Enable Sync" action in Profile. This is the OpenSpec change `local-first-sync-architecture` (currently **active**, not archived — see "OpenSpec routing" below). The authoritative specs are the four delta specs in `openspec/changes/local-first-sync-architecture/specs/` (`local-first-store`, `sync-client`, `entry-provenance`, `lock-screen-controls`); decisions D1–D10 are in `openspec/changes/local-first-sync-architecture/design.md`.
...
Do not claim these are done. All are open tasks in `openspec/changes/local-first-sync-architecture/tasks.md` and absent from the codebase:

- **Undo UI**: `UndoToast` countdown, shake-to-undo (`ShakeHostingController` iOS 15/16 / `.onShake` iOS 17+), supersession UX, and the bulk-delete cap (tasks 3.3–3.6).
...
- **Baseline specs** (`openspec/specs/<capability>/spec.md`): the current merged contract, in force. Two baselines exist today: `app-shell` and `timer-capture-experience`. They were synced from archived changes. **Rule: never edit a baseline spec directly** — behavior changes go through a change with a delta spec.
- **Active deltas** (`openspec/changes/<change>/specs/<capability>/spec.md`): proposed ADDED/MODIFIED requirements not yet in the baseline. The only active change today is `local-first-sync-architecture`; its four delta specs `local-first-store`, `sync-client`, `entry-provenance`, and `lock-screen-controls` define the current local-first contract. Read its `tasks.md` or run `openspec status --change local-first-sync-architecture` for live progress.
- **Archives** (`openspec/changes/archive/<change>/`): completed changes. Their delta specs were folded into the baselines by `openspec archive`; the folder is history. Today: `redesign-track-experience`, `keep-timer-position-on-stop`, `unify-activity-preparation-flow`, `refine-selected-activity-from-track`.
- **Routing**: before touching a behavior, check the active change's tasks and delta specs (`openspec validate --all`, `openspec list`, `openspec status --change local-first-sync-architecture`). When implementing, mark tasks in `tasks.md`. When the active change is complete, run `openspec archive local-first-sync-architecture` to fold deltas into the baselines, then update this context (the "Incomplete / deferred" list shrinks).
