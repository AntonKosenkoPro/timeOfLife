...
- Pull resolves same-name/different-id category collisions by newer-owns-the-name (adopt via identity remap, else skip + heal on push); entry merges prune unknown category ids and keep the remainder; dangling category references drop with a log. Seeding skips present names instead of failing the set.
