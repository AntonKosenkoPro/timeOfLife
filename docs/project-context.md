...
- **History** — a read-only, day-grouped list of committed time entries in the History tab (the `history-entry-list` change, nav bar made persistent by `revert-history-nav-collapse`): `HistoryView` + `HistoryViewModel` with `EntryRow` rows (Variant H layout: first-category icon, name + natural-language duration, category names + timeframe), relative-then-absolute day labels, per-day totals shown on the elevated (pinned) day header, a permanently visible nav bar, and the compact cross-tab timer preserved. Deferred follow-ups (tap-to-detail, filtering, manual entries, delete/undo, provenance labels) live in `docs/history-roadmap.md`.
...
- **Archives** (`openspec/changes/archive/<change>/`): completed changes. Their delta specs were folded into the baselines by `openspec archive`; the folder is history. Today: `redesign-track-experience`, `keep-timer-position-on-stop`, `unify-activity-preparation-flow`, `refine-selected-activity-from-track`, `add-category-management`, `integrate-app-icon`, `collapsing-editor-sheet-headers`, `refine-track-recents`.
