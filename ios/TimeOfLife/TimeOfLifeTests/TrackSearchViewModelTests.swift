...
struct TrackSearchViewModelTests {
    ...
    // MARK: - Configured creation

    @Test("configured creation opens with the trimmed query prefilled")
    func configuredCreationPrefills() async throws {
        let vm = makeViewModel()
        await vm.load()
        vm.activateSearch()
        vm.setSearchQuery("  Gym  ")
        ...
        #expect(vm.search.editor?.draft.name == "Gym")
    }

    @Test("cancelling configured creation preserves the active search and query")
    func cancelConfiguredPreservesSearch() async throws {
        let vm = makeViewModel()
        await vm.load()
        vm.activateSearch()
        vm.setSearchQuery("Gym")
        ...
        vm.cancelConfiguredCreation()
        #expect(vm.search.editor == nil)
        #expect(vm.isSearchActive)
        #expect(vm.search.query == "Gym")
        #expect(vm.state == .idle)
    }

    @Test("saving configured creation prepares the activity without starting")
    func saveConfiguredPrepares() async throws {
        let vm = makeViewModel()
        await vm.load()
        vm.activateSearch()
        vm.setSearchQuery("Gym")
        ...
        let draft = ActivityDraft(name: "Gym", notes: "Leg day")
        let outcome = try await vm.service.prepareActivity(named: draft.name, notes: draft.notes)
        guard case let .created(activity) = outcome else {
            Issue.record("expected created outcome")
            return
        }
        await vm.saveConfiguredCreation(draft: draft, saved: activity)
        guard case let .ready(prepared) = vm.state else {
            Issue.record("expected ready state")
            return
        }
        #expect(prepared.id == activity.id)
        #expect(!vm.isSearchActive)
        #expect(vm.search.editor == nil)
        #expect(!vm.state.isRunning)
    }

    @Test("a configured-save collision presents the explicit choice")
    func configuredCollisionPresentsChoice() async throws {
        let vm = makeViewModel()
        let store = vm.service.store
        ...
        let draft = ActivityDraft(name: "Gym", notes: "Leg day")
        let outcome = try await vm.service.prepareActivity(named: draft.name, notes: draft.notes)
        guard case let .existing(existing) = outcome else {
            Issue.record("expected existing outcome")
            return
        }
        vm.reportConfiguredCollision(existing: existing, draft: draft)
        #expect(vm.search.collision?.existing.id == "a1")
        #expect(vm.search.collision?.draft.notes == "Leg day")
        #expect(vm.search.editor == nil)
    }

    @Test("use existing after collision prepares the existing activity without applying the draft")
    func useExistingAfterCollision() async throws {
        let vm = makeViewModel()
        let store = vm.service.store
        try await store.createActivity(Activity(id: "a1", name: "Gym"))
        await vm.load()
        ...
        #expect(prepared.id == "a1")
        #expect(prepared.notes == nil)
        let stored = try await store.activity(id: "a1")
        #expect(stored?.notes == nil)
        #expect(vm.search.collision == nil)
    }

    @Test("keep editing after collision reopens the editor with the draft intact")
    func keepEditingAfterCollision() async throws {
        let vm = makeViewModel()
        let store = vm.service.store
        try await store.createActivity(Activity(id: "a1", name: "Gym"))
        await vm.load()
        ...
        #expect(vm.search.collision == nil)
        #expect(vm.search.editor?.draft.name == "Gym")
        #expect(vm.search.editor?.draft.notes == "Leg day")
        #expect(vm.state == .idle)
    }
