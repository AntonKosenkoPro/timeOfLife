...
- **Active deltas**: `openspec/changes/account-bound-local-data/` — mandatory sign-in gate, per-account files (`account-bound-store`), per-device sessions (`device-sessions`), same-account sync guard + modified `app-shell`/`local-first-store`/`sync-client`/`lock-screen-controls` deltas. Check its `tasks.md` and `openspec status --change account-bound-local-data` before implementing; mark tasks as you complete them.
