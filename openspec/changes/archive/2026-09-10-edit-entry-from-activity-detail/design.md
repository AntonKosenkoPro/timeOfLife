...
`deleteEntry`-immediate would make a later toast a semantic migration; routing through `UndoBufferStore.enter` now means §8 adds only UI. Needs a small entry-snapshot restore helper mirroring `undoCategoryDeletion` (payload = serialized `TimeEntry`; restore re-inserts + drops the buffer row, no outbox). `ActivityDetailViewModel` gains `performUndo` mirroring categories plus `registerSystemUndo(with:)` (cleared-then-single registration, expiry- and entry-type-checked, re-registering after each undo while a further entry deletion stays restorable); the detail view registers on appear and after every sheet dismissal, so shaking surfaces the DEFAULT system Undo confirmation and confirming restores exactly one entry — the most recent buffered deletion. The registration names the action with the existing localized Delete string (`setActionName`), so the system prompt states what Confirm will undo instead of showing an empty action slot. No custom motion catcher on this surface (an immediate on-shake restore PLUS the system confirmation restored twice: once on shake, once on confirm) and the shared `.onShake` catcher stays untouched so Manage Categories behavior cannot change. The passive `ShakeFirstResponderHost` holds first responder (this sheet has no editable text) and overrides `undoManager` to return the SAME environment instance the view registers with — otherwise the shake resolves to the window's manager, which holds nothing and the prompt never appears. Foreground `commitExpired` is already global so no lifecycle wiring. Confirm copy names no activity; U7 supersession (including cross-surface vs category deletes) is specified, not hidden.
