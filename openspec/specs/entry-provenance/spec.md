# entry-provenance Specification
...
The system SHALL display the provenance of an entry to the user on entry rows. In the History list, non-`manual` entries SHALL show a localized "via <Source>" suffix on the entry row (e.g., "via Screen Time", "via Garmin", "via Widget"). Entries with `source='manual'` SHALL display no source label (they are the default). All labels SHALL use localized strings.
